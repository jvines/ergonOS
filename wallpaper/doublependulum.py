...
def generate(size, seed=0, ensemble=3000, steps=12_000, dt=0.002,
    ...
    # Drawn as PATHS, not points. Each column of bx/by is one pendulum's tip
    # over time; depositing the samples alone draws a dotted line wherever the
    # tip is fast, and a double pendulum is fast exactly at the bottom of every
    # swing -- the part of the curve the eye follows.
    #
    # The last row of each block is carried into the next, so the segment that
    # crosses a block boundary is drawn too. Without it every 1000th step leaves
    # a gap in every trajectory, which at these densities reads as a faint
    ...
    for _ in range(steps):
        ...
        k += 1
        if k == block:
            field += histogram2d(bx, by, size, extent=extent, zoom=1.0,
                                 path=True)
            bx[0], by[0] = bx[-1], by[-1]
            k = 1
    if k > 1:
...
def generate_rods(size, seed=0, ensemble=60, spread=1e-10, dt=0.002,
                  t_first=10.0, t_last=20.0, snapshots=24, **kw):
    """The pendulums THEMSELVES, drawn at successive instants: a fan of rods.

    The trajectory view shows where the lower bob has been. This shows the
    machine: every pendulum in the ensemble drawn as its two rods -- pivot to
    first bob, first bob to second -- at `snapshots` instants between t_first
    and t_last. Released 1e-10 rad apart, the ensemble is a single pendulum for
    the first stretch, so the early snapshots overlay exactly and draw one
    bright rod; past the Lyapunov time they splay, and the later snapshots fan
    out. What you see is sensitive dependence as a picture: one configuration
    becoming many.

    Nothing is drawn but rods. No traces, no haze -- which is exactly what made
    the trajectory version read as mush once the ensemble had decohered.
    """
    from lib import histogram2d
    names = sorted(RELEASES)
    th1, th2 = RELEASES[names[seed % len(RELEASES)]]
    ...
    # Both rods are unit length, so every point of every pendulum lies within
    # radius 2 of the pivot. Framed on that disc -- a fixed window, so the fan
    # sits where the pivot is rather than wherever the tips happened to go.
    extent = (-2.15, 2.15, -2.15, 2.15)

    stops = np.linspace(t_first, t_last, snapshots)
    w, h = size
    field = np.zeros((h, w))
    t = 0.0
    for ts in stops:
        ...
        x2, y2 = x1 + np.sin(y[1]), y1 - np.cos(y[1])
        # (3, ensemble): each COLUMN is one pendulum as a 3-point polyline,
        # pivot -> bob 1 -> bob 2, drawn as a continuous path.
        xs = np.stack([np.zeros(ensemble), x1, x2])
        # NEGATED: the raster's rows grow downward while physical y grows up,
        # so without this every pendulum hangs toward the top of the screen.
        # Invisible on a cloud of trajectory points; unmistakable once there
        # is a pivot and a rod.
        ys = -np.stack([np.zeros(ensemble), y1, y2])
